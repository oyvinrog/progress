...
class TestGamificationCoreLoop:
    def test_completion_rewards_and_level_fields(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel

        task_model = TaskModel()
        task_model.addTask("One", -1)
        task_model.addTask("Two", -1)
        for task in task_model._tasks:
        ...
        now = {"value": 10 * 3600 + 5}
        monkeypatch.setattr("task_model.time.time", lambda: now["value"])

        task_model.toggleComplete(0, True)
        assert project_manager.gamificationXp == 13
        assert project_manager.gamificationCurrentStreakHours == 1
        assert project_manager.gamificationHourlyCompletions == 1

        task_model.toggleComplete(1, True)
        assert project_manager.gamificationXp == 23
        assert project_manager.gamificationCurrentStreakHours == 1
        assert project_manager.gamificationHourlyCompletions == 2
        assert project_manager.gamificationLevel == 1
        assert project_manager.gamificationXpIntoLevel == 23
        assert project_manager.gamificationXpForNextLevel == 100
        assert project_manager.gamificationLevelProgress == pytest.approx(0.23)

    def test_completion_awards_coins(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel
        ...
        now = {"value": 12 * 3600 + 10}
        monkeypatch.setattr("task_model.time.time", lambda: now["value"])
        monkeypatch.setattr("task_model.random.random", lambda: 0.99)

        task_model.toggleComplete(0, True)
        assert project_manager.gamificationCoins == 7
        assert project_manager.gamificationPouchCount == 0

        task_model.toggleComplete(1, True)
        assert project_manager.gamificationCoins == 12

    def test_shop_buy_and_use_consumable(self, app):
        from task_model import ProjectManager, TabModel
        ...
        project_manager._gamification_state.coins_total = 40
        assert project_manager.buyShopItem("focus_potion") is True
        assert project_manager.gamificationCoins == 22
        assert project_manager.getPouchItemCount("focus_potion") == 1

        assert project_manager.usePouchItem("focus_potion") is True
        assert project_manager.getPouchItemCount("focus_potion") == 0
        assert project_manager._gamification_state.active_effects.get("focus_next_completion", 0) == 1

    def test_shop_rewards_catalog_and_purchase(self, app):
        from task_model import ProjectManager, TabModel
        ...
        shop_by_id = {item["id"]: item for item in project_manager.gamificationShopItems}
        expected = {
            "cola": ("Cola", 50, "🥤"),
            "snus": ("Snus", 100, "🧊"),
            "bolle": ("Bolle", 100, "🥐"),
            "cinema": ("Cinema", 500, "🎬"),
            "espresso_shot": ("Espresso Shot", 20, "☕"),
            "cocoa": ("Cocoa", 40, "🍫"),
        }

        for item_id, (name, price, icon) in expected.items():
            assert item_id in shop_by_id
            assert shop_by_id[item_id]["name"] == name
            assert shop_by_id[item_id]["price"] == price
            assert shop_by_id[item_id]["icon"] == icon
            assert shop_by_id[item_id]["itemType"] == "reward"

        project_manager._gamification_state.coins_total = 60
        assert project_manager.buyShopItem("cola") is True
        assert project_manager.gamificationCoins == 10
        assert project_manager.getPouchItemCount("cola") == 1
        assert project_manager.buyShopItem("cocoa") is False

    def test_shop_reward_purchase_blocked_when_pouch_full(self, app):
        from task_model import ProjectManager, TabModel
        ...
        project_manager._gamification_state.coins_total = 1000
        project_manager._gamification_state.pouch_capacity = 2
        assert project_manager._addPouchItem("focus_potion")
        assert project_manager._addPouchItem("streak_shield")
        assert project_manager.gamificationPouchCount == 2

        assert project_manager.buyShopItem("cinema") is False
        assert project_manager.gamificationCoins == 1000
        assert project_manager.gamificationPouchCount == 2

    def test_shop_reward_item_can_be_used_from_pouch(self, app):
        from task_model import ProjectManager, TabModel
        ...
        project_manager._gamification_state.coins_total = 50
        assert project_manager.buyShopItem("espresso_shot") is True
        assert project_manager.getPouchItemCount("espresso_shot") == 1

        assert project_manager.usePouchItem("espresso_shot") is True
        assert project_manager.getPouchItemCount("espresso_shot") == 0

    def test_streak_shield_prevents_hour_gap_reset_once(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel
        ...
        assert project_manager._addPouchItem("streak_shield")
        assert project_manager.usePouchItem("streak_shield") is True
        ...
        task_model.toggleComplete(1, True)
        assert project_manager.gamificationCurrentStreakHours == 1
        assert project_manager._gamification_state.active_effects.get("streak_shield_charges", 0) == 0

    def test_lucky_charm_consumes_charges_and_enables_drop(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel

        task_model = TaskModel()
        task_model.addTask("One", -1)
        task_model._tasks[0].start_time = None
        diagram_model = DiagramModel(task_model=task_model)
        tab_model = TabModel()
        project_manager = ProjectManager(task_model, diagram_model, tab_model)

        assert project_manager._addPouchItem("lucky_charm")
        assert project_manager.usePouchItem("lucky_charm") is True

        monkeypatch.setattr("task_model.time.time", lambda: 9 * 3600 + 1)
        monkeypatch.setattr("task_model.random.random", lambda: 0.20)
        monkeypatch.setattr("task_model.random.uniform", lambda a, b: 1.0)

        task_model.toggleComplete(0, True)
        assert project_manager.getPouchItemCount("focus_potion") == 1
        assert project_manager._gamification_state.active_effects.get("lucky_charm_remaining", 0) == 2

    def test_pouch_capacity_blocks_shop_purchase(self, app):
        from task_model import ProjectManager, TabModel
        ...
        assert project_manager.gamificationPouchCount == 2
        assert project_manager.buyShopItem("lucky_charm") is False

    def test_pouch_slots_expose_full_capacity(self, app):
        from task_model import ProjectManager, TabModel
        ...
        project_manager._gamification_state.pouch_capacity = 5
        assert project_manager._addPouchItem("focus_potion")
        assert project_manager._addPouchItem("streak_shield")

        slots = project_manager.gamificationPouchSlots
        assert len(slots) == 5
        assert slots[0]["filled"] is True
        assert slots[1]["filled"] is True
        assert slots[2]["filled"] is False
        assert project_manager.gamificationPouchFillPercent == pytest.approx(0.4)

    def test_hourly_streak_consecutive_and_gap_reset(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel

        task_model = TaskModel()
        task_model.addTask("One", -1)
        task_model.addTask("Two", -1)
        task_model.addTask("Three", -1)
        for task in task_model._tasks:
            task.start_time = None
        diagram_model = DiagramModel(task_model=task_model)
        tab_model = TabModel()
        ...
        task_model.toggleComplete(0, True)
        assert project_manager.gamificationCurrentStreakHours == 1
        assert project_manager.gamificationBestStreakHours == 1

        now["value"] = 11 * 3600 + 1
        task_model.toggleComplete(1, True)
        assert project_manager.gamificationXp == 27
        assert project_manager.gamificationCurrentStreakHours == 2
        assert project_manager.gamificationBestStreakHours == 2
        assert project_manager.gamificationHourlyCompletions == 1

        now["value"] = 13 * 3600 + 1
        task_model.toggleComplete(2, True)
        assert project_manager.gamificationXp == 40
        assert project_manager.gamificationCurrentStreakHours == 1
        assert project_manager.gamificationBestStreakHours == 2
        assert project_manager.gamificationHourlyCompletions == 1

        now["value"] = 14 * 3600 + 1
        assert project_manager.gamificationHourlyCompletions == 0
        assert project_manager.gamificationHourlyGoalProgress == 0.0

    def test_unchecking_task_does_not_deduct_xp(self, app, monkeypatch):
        from task_model import ProjectManager, TabModel
        ...
        monkeypatch.setattr("task_model.time.time", lambda: 10 * 3600 + 5)
        task_model.toggleComplete(0, True)
        xp_after_complete = project_manager.gamificationXp
        task_model.toggleComplete(0, False)
        assert project_manager.gamificationXp == xp_after_complete

    def test_legacy_load_without_gamification_does_not_backfill_xp(self, app, tmp_path):
        import json
        from task_model import ProjectManager, TabModel

        project_data = {
            "version": "1.0",
            "tasks": {"tasks": [{"title": "Done task", "completed": True, "time_spent": 15.0}]},
            "diagram": {"items": [], "edges": [], "strokes": [], "current_task_index": -1},
        }
        project_file = tmp_path / "legacy_no_gamification.progress"
        ...
        assert project_manager.gamificationXp == 0
        assert project_manager.gamificationCurrentStreakHours == 0

    def test_gamification_roundtrip_persists(self, app, tmp_path, monkeypatch):
        from task_model import ProjectManager, TabModel
        ...
        monkeypatch.setattr("task_model.time.time", lambda: 9 * 3600 + 2)
        monkeypatch.setattr("task_model.random.random", lambda: 0.99)
        task_model.toggleComplete(0, True)
        expected_xp = project_manager.gamificationXp
        expected_streak = project_manager.gamificationCurrentStreakHours
        expected_coins = project_manager.gamificationCoins

        project_manager._gamification_state.coins_total = 200
        assert project_manager.buyShopItem("focus_potion")
        assert project_manager.usePouchItem("focus_potion")

        project_file = tmp_path / "gamification_roundtrip.progress"
        project_manager.saveProject(str(project_file))
        ...
        assert project_manager2.gamificationXp == expected_xp
        assert project_manager2.gamificationCurrentStreakHours == expected_streak
        assert project_manager2.gamificationLevel >= 1
        assert project_manager2.gamificationCoins >= expected_coins
        assert project_manager2._gamification_state.active_effects.get("focus_next_completion", 0) >= 1

    def test_has_unsaved_changes_after_completion(self, app, tmp_path, monkeypatch):
        from task_model import ProjectManager, TabModel

        task_model = TaskModel()
        task_model.addTask("Unsaved", -1)
        task_model._tasks[0].start_time = None
        diagram_model = DiagramModel(task_model=task_model)
        tab_model = TabModel()
        project_manager = ProjectManager(task_model, diagram_model, tab_model)

        project_file = tmp_path / "gamification_unsaved.progress"
        project_manager.saveProject(str(project_file))
        assert project_manager.hasUnsavedChanges() is False

        monkeypatch.setattr("task_model.time.time", lambda: 8 * 3600 + 1)
        task_model.toggleComplete(0, True)
        assert project_manager.hasUnsavedChanges() is True
